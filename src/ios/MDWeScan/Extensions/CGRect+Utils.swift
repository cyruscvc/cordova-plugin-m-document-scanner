...
extension CGRect {
    ...
    func mdwScaleAndCenter(withRatio ratio: CGFloat) -> CGRect {
        let mdwScaleTransform = CGAffineTransform(scaleX: ratio, y: ratio)
        let scaledRect = applying(mdwScaleTransform)

        let mdwTranslateTransform = CGAffineTransform(
            translationX: origin.x * (1 - ratio) + (width - scaledRect.width) / 2.0,
            y: origin.y * (1 - ratio) + (height - scaledRect.height) / 2.0
        )
        let translatedRect = scaledRect.applying(mdwTranslateTransform)
