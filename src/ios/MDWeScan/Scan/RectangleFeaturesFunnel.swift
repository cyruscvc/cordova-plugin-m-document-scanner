//
//  MDWRectangleFeaturesFunnel.swift
//  WeScan
//
...
/// `MDWRectangleFeaturesFunnel` is used to improve the confidence of the detected rectangles.
/// Feed rectangles to a `MDWRectangleFeaturesFunnel` instance, and it will call the completion block with a rectangle whose confidence is high enough to be displayed.
final class MDWRectangleFeaturesFunnel {
    ...
    private final class RectangleMatch: NSObject {
        ...
        let rectangleFeature: MDWQuadrilateral
        ...
        init(rectangleFeature: MDWQuadrilateral) {
        ...
        func matches(_ rectangle: MDWQuadrilateral, withThreshold threshold: CGFloat) -> Bool {
            return rectangleFeature.mdwIsWithin(threshold, ofRectangleFeature: rectangle)
        }
    }
    ...
    private func bestRectangle(withCurrentlyDisplayedRectangle currentRectangle: MDWQuadrilateral?) -> RectangleMatch? {
    ...
    private func breakTie(between rect1: RectangleMatch, rect2: RectangleMatch, currentRectangle: MDWQuadrilateral) -> RectangleMatch {
        if rect1.rectangleFeature.mdwIsWithin(matchingThreshold, ofRectangleFeature: currentRectangle) {
            return rect1
        } else if rect2.rectangleFeature.mdwIsWithin(matchingThreshold, ofRectangleFeature: currentRectangle) {
            return rect2
        }
