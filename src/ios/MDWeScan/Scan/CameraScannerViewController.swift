//
//  MDWCameraScannerViewController.swift
//  WeScan
//
...
public protocol MDWCameraScannerViewOutputDelegate: AnyObject {
    func captureImageFailWithError(error: Error)
    func captureImageSuccess(image: UIImage, withQuad quad: MDWQuadrilateral?)
}

/// A view controller that manages the camera module and auto capture of rectangle shape of document
/// The `MDWCameraScannerViewController` class is individual camera view include touch for focus, flash control,
/// capture control and auto detect rectangle shape of object.
public final class MDWCameraScannerViewController: UIViewController {

    /// The status of auto scan.
    public var isAutoScanEnabled: Bool = MDWCaptureSession.current.isAutoScanEnabled {
        didSet {
            MDWCaptureSession.current.isAutoScanEnabled = isAutoScanEnabled
        }
    }

    /// The callback to caller view to send back success or fail.
    public weak var delegate: MDWCameraScannerViewOutputDelegate?
    ...
    private func setupView() {
        ...
        captureSessionManager = MDWCaptureSessionManager(videoPreviewLayer: videoPreviewLayer)
...
extension MDWCameraScannerViewController: MDWRectangleDetectionDelegateProtocol {
    ...
    func captureSessionManager(_ captureSessionManager: MDWCaptureSessionManager,
    ...
    func captureSessionManager(_ captureSessionManager: MDWCaptureSessionManager,
                               didDetectQuad quad: MDWQuadrilateral?,
                               _ imageSize: CGSize) {
