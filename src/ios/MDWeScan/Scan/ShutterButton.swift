...
final class MDWShutterButton: UIControl {
    ...
    private func pathForOuterRing(inRect rect: CGRect) -> UIBezierPath {
        ...
        let innerRect = rect.mdwScaleAndCenter(withRatio: outerRingRatio)
    ...
    private func pathForInnerCircle(inRect rect: CGRect) -> UIBezierPath {
        let rect = rect.mdwScaleAndCenter(withRatio: innerRingRatio)
        let path = UIBezierPath(ovalIn: rect)
