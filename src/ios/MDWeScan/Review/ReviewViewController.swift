...
/// The `MDWReviewViewController` offers an interface to review the image after it
/// has been cropped and deskewed according to the passed in quadrilateral.
final class MDWReviewViewController: UIViewController {
    ...
    private lazy var rotateButton: UIBarButtonItem = {
        let image = UIImage(mdwSystemName: "rotate.right", named: "rotate", in: Bundle(for: MDWScannerViewController.self), compatibleWith: nil)
    ...
    private let results: MDWImageScannerResults

    // MARK: - Life Cycle

    init(results: MDWImageScannerResults) {
    ...
    @objc private func reloadImage() {
        if enhancedImageIsAvailable, isCurrentlyDisplayingEnhancedImage {
            imageView.image = results.enhancedScan?.image.mdwRotated(by: rotationAngle) ?? results.enhancedScan?.image
        } else {
            imageView.image = results.croppedScan.image.mdwRotated(by: rotationAngle) ?? results.croppedScan.image
        }
    }
