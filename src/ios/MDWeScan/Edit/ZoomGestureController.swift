//
//  MDWZoomGestureController.swift
//  WeScan
//
...
final class MDWZoomGestureController {

    private let image: UIImage
    private let quadView: MDWQuadrilateralView
    private var previousPanPosition: CGPoint?
    private var closestCorner: MDWCornerPosition?

    init(image: UIImage, quadView: MDWQuadrilateralView) {
        self.image = image
    ...
    @objc func handle(pan: UIGestureRecognizer) {
        ...
        let closestCorner = self.closestCorner ?? position.mdwClosestCornerFrom(quad: drawnQuad)
        ...
        guard let zoomedImage = image.mdwScaledImage(
