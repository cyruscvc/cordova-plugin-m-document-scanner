...
public struct MDWQuadrilateral: MDWTransformable {
    ...
    /// The path of the MDWQuadrilateral as a `UIBezierPath`
    ...
    /// The perimeter of the MDWQuadrilateral
    var perimeter: Double {
        let perimeter = topLeft.mdwDistanceTo(point: topRight)
            + topRight.mdwDistanceTo(point: bottomRight)
            + bottomRight.mdwDistanceTo(point: bottomLeft)
            + bottomLeft.mdwDistanceTo(point: topLeft)
    ...
    func applying(_ transform: CGAffineTransform) -> MDWQuadrilateral {
        let quadrilateral = MDWQuadrilateral(
    ...
    func mdwIsWithin(_ distance: CGFloat, ofRectangleFeature rectangleFeature: MDWQuadrilateral) -> Bool {

        let topLeftRect = topLeft.mdwSurroundingSquare(withSize: distance)
        if !topLeftRect.contains(rectangleFeature.topLeft) {
            return false
        }

        let topRightRect = topRight.mdwSurroundingSquare(withSize: distance)
        if !topRightRect.contains(rectangleFeature.topRight) {
            return false
        }

        let bottomRightRect = bottomRight.mdwSurroundingSquare(withSize: distance)
        if !bottomRightRect.contains(rectangleFeature.bottomRight) {
            return false
        }

        let bottomLeftRect = bottomLeft.mdwSurroundingSquare(withSize: distance)
    ...
    /// - Returns: The newly scaled and potentially mdwRotated quadrilateral.
    func scale(_ fromSize: CGSize, _ toSize: CGSize, withRotationAngle rotationAngle: CGFloat = 0.0) -> MDWQuadrilateral {
        var invertedFromSize = fromSize
        let mdwRotated = rotationAngle != 0.0

        if mdwRotated && rotationAngle != CGFloat.pi {
        ...
        if mdwRotated {
            ...
            let translationTransform = CGAffineTransform.mdwTranslateTransform(
...
extension MDWQuadrilateral {

    /// Converts the current to the mdwCartesian coordinate system (where 0 on the y axis is at the bottom).
    ///
    /// - Parameters:
    ///   - height: The height of the rect containing the quadrilateral.
    /// - Returns: The same quadrilateral in the mdwCartesian coordinate system.
    func toCartesian(withHeight height: CGFloat) -> MDWQuadrilateral {
        let topLeft = self.topLeft.mdwCartesian(withHeight: height)
        let topRight = self.topRight.mdwCartesian(withHeight: height)
        let bottomRight = self.bottomRight.mdwCartesian(withHeight: height)
        let bottomLeft = self.bottomLeft.mdwCartesian(withHeight: height)

        return MDWQuadrilateral(topLeft: topLeft, topRight: topRight, bottomRight: bottomRight, bottomLeft: bottomLeft)
    }
}

extension MDWQuadrilateral: Equatable {
    public static func == (lhs: MDWQuadrilateral, rhs: MDWQuadrilateral) -> Bool {
